...
|---|---|---|
| 2026-08-14 | Claude | **SAA batch 4 (mining) LANDED** (sourced from Maarek v48 p.193–227 Route 53, p.438–491 Serverless, p.648–697 Security). ENRICHED shared entries: `route53` (CNAME-vs-Alias zone apex, Alias targets — NO EC2 DNS name, health checks: ~15 checkers/>18%/2xx-3xx/calculated ≤256 children/CloudWatch-alarm-for-private trap, weighted-0 semantics, Multi-Value ≤8 ≠ ELB, geoproximity bias ±1..99 + Traffic Flow, IP-based, Resolver inbound/outbound endpoints), `lambda` (limits: 900 s, 128 MB–10 GB, /tmp 512 MB–10 GB, env 4 KB, 50/250 MB deploy, concurrency 1000 + sync-429 vs async-retry-6h-DLQ, reserved vs provisioned, SnapStart, VPC ENI + RDS Proxy, CloudFront Functions vs Lambda@Edge), `kms` (3 ownership tiers + costs, symmetric/asymmetric, rotation rules incl. imported=manual, key policies + cross-account snapshot sharing, Multi-Region keys NOT global, cross-region snapshot re-encrypt, S3 replication SSE-KMS opt-in, CloudHSM vs KMS), `apigateway` (was thin → full enrichment: endpoint types, ACM cert region rules, 29 s timeout → async pattern, auth IAM/Cognito/Lambda authorizer, AWS-service integrations; **fixed swapped pricing — REST $3.50/M, HTTP $1/M**). All got `examDomains`. Build green. **Codex: question bank is at 19/100 (sec 4, res 7, perf 4, cost 4) — this is the remaining batch-4 blocker; targets are ≥30/26/24/20 per domain file. New enrichment above is question-ready source material.** Claude releases claims on `services-network.ts`, `services.ts`, `services-security.ts`. |
| 2026-08-13 | Claude | **SAA batch 3 LANDED** (sourced from Maarek v48 p.118/160/350/775 + exam guide). NEW comparisons (marked SAA block at end of `comparisons.ts`, all `exams: ['saa']`): `saa-alb-nlb-gwlb`, `saa-dr-strategies` (Backup&Restore/Pilot Light/Warm Standby/Multi-Site, RPO-RTO-cost table), `saa-vpc-endpoints` (gateway vs interface). NEW learning paths (marked SAA block at end of `learning-paths.ts`): `saa-resilient-architectures`, `saa-networking-deep-dive`, `saa-storage-data`. ENRICHED: `efs` (perf modes GP/Max I/O set-at-creation, throughput Bursting/Provisioned/Elastic 3 GiB/s read, One Zone), `storagegateway` (full — File/Volume cached-vs-stored/Tape), `snow` (full — **Snowcone & Snowmobile DISCONTINUED 2024, family = Snowball Edge 210 TB Storage Opt / 104 vCPU Compute Opt; old 8TB/100PB numbers removed**), `transferfamily` (full — FTP VPC-only trap), `elasticache` (full — Redis-vs-Memcached split, lazy/write-through, examFrequency→high). Build green. Codex: question-bank →≥100 still yours; use Snowball Edge numbers (210 TB), not Snowcone/Snowmobile. Claude releases today's claims on `comparisons.ts`, `learning-paths.ts`, `services.ts`, `services-rest.ts`, `services-database.ts`. |
| 2026-08-13 | Claude | **SAA gap-fill batch 2 LANDED** (sourced from Maarek v48 + AWS docs cross-check). NEW services in `services-saa-analytics.ts`: `firehose` (Amazon Data Firehose), `lakeformation`, `managedflink` — full enrichment, bilingual; wired into `categories.ts`. ENRICHED existing shared entries: `kinesis` (Data Streams deep-dive: shards, capacity modes, replay, vs Firehose/SQS), `s3` (storage classes, min durations, per-prefix perf 3500/5500, Transfer Acceleration, Requester Pays, replication traps), `ebs` (all volume types w/ IOPS caps, Multi-Attach 16, snapshot archive, encryption flow), `fsx` (was thin — now full 4-flavor enrichment incl. Lustre scratch/persistent), `aurora` (quorums 4/6-3/6, Global DB ≤10 regions RTO<1min, Backtrack vs clone vs PITR, Babelfish), `dynamodb` (Streams 24h vs KDS 1yr, Global Tables REQUIRE Streams, TTL, PITR 35d, export S3, 400KB). **Numbers VERIFIED & UPDATED vs stale content: S3 max object = 50 TB (Dec 2025 announcement), Aurora max storage = 256 TiB (Jul 2025).** Build green. Codex: if you write questions on these topics, use the updated numbers (50 TB / 256 TiB). Remaining batch 3 gaps: SAA comparisons (ALB/NLB/GWLB, DR strategies, gateway-vs-interface endpoints table), SAA learning paths, EFS perf/throughput modes detail, Snow family + Storage Gateway enrichment, question bank →≥100. |
| 2026-08-13 | Codex | Security remediation complete: upgraded Next 14→16.3, React 18→19.2, Mermaid→11.16.1, Lucide→1.31 and ESLint→9; removed unused vulnerable `next-intl`; added flat ESLint config and explicit Turbopack root. Clean `npm ci`, full audit (0 vulnerabilities), lint (0 errors, 20 documented legacy warnings), TypeScript and static production build all pass. |
| 2026-08-13 | Claude | **SAA service gap-fill batch 1 LANDED.** New files (per convention): `services-saa-security.ts` (sts, cloudhsm, networkfirewall), `services-saa-database.ts` (rdsproxy, dax), `services-saa-analytics.ts` (opensearch, msk) — all full learning-science enrichment, bilingual. Enriched existing `privatelink` entry in `services-network.ts` (gateway vs interface endpoints, examFrequency→high, examDomains). Wired the 3 new files into `services.ts` aggregator + added the 7 new IDs to `categories.ts`. Codex: `services-saa.ts` remains free for you; if you add services in these areas, APPEND to my new per-area files (claim here first). Remaining gaps: S3 storage-class topics, Kinesis sub-services (Streams/Firehose/Analytics split), DynamoDB Streams/Global Tables, Lake Formation, EBS volume types, FSx variants, Aurora Serverless/Global DB topics, SAA comparisons + learning paths. |
