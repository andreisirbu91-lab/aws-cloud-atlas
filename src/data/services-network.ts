...
export const networkServices: Service[] = [
  ...
  },
  {
    ...
    examTips: [
      ...
      { key: 'cname_vs_alias', content: { en: 'CNAME cannot exist at the zone apex (example.com) — Alias can, and it points at AWS resources.', ro: 'CNAME nu poate exista la zone apex (example.com) — Alias da, și țintește resurse AWS.' } },
      { key: 'multivalue', content: { en: 'Multi-Value: up to 8 healthy records per answer — NOT a substitute for a load balancer.', ro: 'Multi-Value: până la 8 înregistrări sănătoase per răspuns — NU e un substitut de load balancer.' } },
      { key: 'geoproximity', content: { en: 'Geoproximity: shift traffic with a bias (1..99 expand, -1..-99 shrink); requires Traffic Flow.', ro: 'Geoproximity: muți traficul cu un bias (1..99 extinde, -1..-99 restrânge); necesită Traffic Flow.' } },
    ],
    ...
    howItWorks: [
      ...
      { en: 'Alias records map a name to an AWS resource (ALB, CloudFront, API GW, S3 website…) and track its IP changes automatically; you cannot set their TTL.', ro: 'Alias records mapează un nume la o resursă AWS (ALB, CloudFront, API GW, site S3…) și urmăresc automat schimbările de IP; nu le poți seta TTL-ul.' },
      { en: 'Hybrid DNS: Resolver INBOUND endpoints let on-premises resolvers query your private hosted zones; OUTBOUND endpoints forward VPC queries to on-premises DNS.', ro: 'DNS hibrid: Resolver INBOUND endpoints lasă resolver-ele on-premises să interogheze private hosted zones; OUTBOUND endpoints trimit interogările din VPC către DNS-ul on-premises.' },
    ],
    keyFacts: [
      ...
      { en: 'CNAME works only on subdomains (www.example.com); at the zone apex (example.com) you MUST use an Alias record — free, health-check aware, A/AAAA type.', ro: 'CNAME merge doar pe subdomenii (www.example.com); la zone apex (example.com) TREBUIE să folosești Alias — gratuit, cu health check, tip A/AAAA.' },
    ...
    keyNumbers: [
      ...
      { label: { en: 'Routing policies', ro: 'Politici de routing' }, value: { en: '8 (Simple, Weighted, Latency, Failover, Geolocation, Geoproximity, Multi-Value, IP-based)', ro: '8 (Simple, Weighted, Latency, Failover, Geolocation, Geoproximity, Multi-Value, IP-based)' } },
      { label: { en: 'Health check interval', ro: 'Interval health check' }, value: { en: '30 s default (10 s fast, costs more)', ro: '30 s implicit (10 s rapid, cost mai mare)' } },
      { label: { en: 'Healthy verdict', ro: 'Verdict sănătos' }, value: { en: '>18% of ~15 global checkers report healthy; only 2xx/3xx pass', ro: '>18% din ~15 checkere globale raportează sănătos; doar 2xx/3xx trec' } },
      { label: { en: 'Calculated health check children', ro: 'Copii calculated health check' }, value: { en: 'up to 256 (AND / OR / NOT)', ro: 'până la 256 (AND / OR / NOT)' } },
      { label: { en: 'Multi-Value records per answer', ro: 'Înregistrări Multi-Value per răspuns' }, value: { en: 'up to 8 healthy', ro: 'până la 8 sănătoase' } },
    ],
    whenToUse: [
    ...
    examTraps: [
      ...
      { en: '"Point the ROOT domain (example.com) at an ALB/CloudFront" → Alias record. A CNAME at the zone apex is invalid — classic wrong answer.', ro: '„Îndreaptă domeniul RĂDĂCINĂ (example.com) spre un ALB/CloudFront” → Alias record. Un CNAME la zone apex e invalid — răspuns greșit clasic.' },
      { en: 'You CANNOT create an Alias record to an EC2 public DNS name — Alias targets are ELB, CloudFront, API GW, S3 website, Global Accelerator, Beanstalk, VPC interface endpoints, other Route 53 records.', ro: 'NU poți crea un Alias către DNS-ul public al unui EC2 — țintele Alias sunt ELB, CloudFront, API GW, site S3, Global Accelerator, Beanstalk, VPC interface endpoints, alte înregistrări Route 53.' },
      { en: 'Health checkers live OUTSIDE your VPC — to health-check a PRIVATE resource, create a CloudWatch metric + alarm and a health check that monitors that alarm.', ro: 'Health checker-ele sunt ÎN AFARA VPC-ului — pentru o resursă PRIVATĂ, creezi o metrică + alarmă CloudWatch și un health check care monitorizează alarma.' },
      { en: 'Weighted routing: weight 0 = stop sending traffic to that record; if ALL records have weight 0, all are returned equally.', ro: 'Weighted routing: weight 0 = oprești traficul spre acea înregistrare; dacă TOATE au weight 0, toate sunt returnate egal.' },
    ],
    retrievalQuestions: [
      ...
      { q: { en: 'CNAME vs Alias — which one works at the zone apex and why prefer Alias for AWS resources?', ro: 'CNAME vs Alias — care merge la zone apex și de ce preferi Alias pentru resurse AWS?' }, a: { en: 'Alias works at the apex (CNAME does not). Alias is free, tracks the resource\'s IP changes, supports health checks, and you cannot set its TTL.', ro: 'Alias merge la apex (CNAME nu). Alias e gratuit, urmărește schimbările de IP ale resursei, suportă health checks și nu-i poți seta TTL-ul.' } },
      { q: { en: 'How do you health-check a private resource that Route 53 checkers cannot reach?', ro: 'Cum faci health check unei resurse private la care checker-ele Route 53 nu ajung?' }, a: { en: 'Create a CloudWatch metric + alarm for it, then a Route 53 health check that monitors the CloudWatch alarm.', ro: 'Creezi o metrică + alarmă CloudWatch pentru ea, apoi un health check Route 53 care monitorizează alarma CloudWatch.' } },
      { q: { en: 'What do Route 53 Resolver inbound and outbound endpoints do?', ro: 'Ce fac endpoint-urile Resolver inbound și outbound din Route 53?' }, a: { en: 'Inbound: on-premises DNS resolvers can resolve names in your VPC/private hosted zones. Outbound: the VPC Resolver forwards queries for on-premises domains to your DNS servers (over VPN/Direct Connect).', ro: 'Inbound: resolver-ele DNS on-premises pot rezolva nume din VPC/private hosted zones. Outbound: Resolver-ul din VPC trimite interogările pentru domenii on-premises către serverele tale DNS (prin VPN/Direct Connect).' } },
      { q: { en: 'What is the bias in Geoproximity routing?', ro: 'Ce e bias-ul din routing-ul Geoproximity?' }, a: { en: 'A value that grows (1..99) or shrinks (-1..-99) the geographic area a resource attracts traffic from; requires Route 53 Traffic Flow.', ro: 'O valoare care mărește (1..99) sau micșorează (-1..-99) zona geografică din care o resursă atrage trafic; necesită Route 53 Traffic Flow.' } },
    ],
    diagram: {
      steps: [
  ...
  },
  {
    ...
    examTips: [
      ...
      { key: 'endpoint_types', content: { en: 'Endpoint types: Edge-Optimized (default, via CloudFront), Regional, Private (interface VPC endpoint).', ro: 'Tipuri de endpoint: Edge-Optimized (implicit, prin CloudFront), Regional, Private (interface VPC endpoint).' } },
      { key: 'acm_region', content: { en: 'Custom domain certs: Edge-Optimized → ACM cert in us-east-1; Regional → cert in the API\'s region.', ro: 'Certificate pentru domenii custom: Edge-Optimized → cert ACM în us-east-1; Regional → cert în regiunea API-ului.' } },
    ],
    pricing: { en: '$3.50/M requests (REST API) or $1.00/M (HTTP API — cheaper)', ro: '$3.50/M cereri (REST API) sau $1.00/M (HTTP API — mai ieftin)' },
    connections: ['lambda', 'cognito', 'iam', 'cloudwatch'],
    docsUrl: 'https://docs.aws.amazon.com/apigateway/',
    visual: { color: 'hsl(190, 90%, 50%)', icon: 'network' },
    examDomains: ['design-resilient', 'design-secure'],
    howItWorks: [
      { en: 'Clients call your API through API Gateway, which handles auth, throttling, caching and versioning before the request reaches the backend.', ro: 'Clienții apelează API-ul prin API Gateway, care se ocupă de auth, throttling, caching și versionare înainte ca cererea să ajungă la backend.' },
      { en: 'Three integration targets: a Lambda function (classic serverless REST), any HTTP endpoint (on-premises, ALB), or an AWS service API (e.g. put a message directly in SQS or Kinesis).', ro: 'Trei ținte de integrare: o funcție Lambda (REST serverless clasic), orice endpoint HTTP (on-premises, ALB) sau un API de serviciu AWS (ex. pui un mesaj direct în SQS sau Kinesis).' },
      { en: 'The endpoint type decides how clients reach it: Edge-Optimized routes through CloudFront edge locations, Regional serves same-region clients, Private is reachable only via an interface VPC endpoint.', ro: 'Tipul de endpoint decide cum ajung clienții la el: Edge-Optimized rutează prin edge locations CloudFront, Regional servește clienți din aceeași regiune, Private e accesibil doar printr-un interface VPC endpoint.' },
      { en: 'User auth plugs in as IAM (internal apps), Cognito User Pools (external/mobile users) or a Lambda (custom) authorizer.', ro: 'Autentificarea se face prin IAM (aplicații interne), Cognito User Pools (utilizatori externi/mobili) sau un Lambda (custom) authorizer.' },
    ],
    keyFacts: [
      { en: 'Serverless front door for APIs: REST API (full features), HTTP API (cheaper, faster, fewer features) and WebSocket API for two-way real-time.', ro: 'Ușa serverless pentru API-uri: REST API (funcții complete), HTTP API (mai ieftin, mai rapid, mai puține funcții) și WebSocket API pentru real-time bidirecțional.' },
      { en: 'Private APIs are exposed only through an interface VPC endpoint, with a resource policy defining who may call them.', ro: 'API-urile Private sunt expuse doar printr-un interface VPC endpoint, cu o resource policy care definește cine le poate apela.' },
      { en: 'Built-in request throttling and API keys with usage plans protect the backend from overload.', ro: 'Throttling built-in și API keys cu usage plans protejează backend-ul de supraîncărcare.' },
      { en: 'Response caching at the stage level reduces backend calls and latency.', ro: 'Caching-ul de răspunsuri la nivel de stage reduce apelurile spre backend și latența.' },
      { en: 'It can expose AWS services directly (SQS, Kinesis, Step Functions) — no Lambda glue needed just to add auth/rate limiting.', ro: 'Poate expune servicii AWS direct (SQS, Kinesis, Step Functions) — fără Lambda intermediar doar ca să adaugi auth/rate limiting.' },
    ],
    keyNumbers: [
      { label: { en: 'Integration timeout', ro: 'Timeout integrare' }, value: { en: '29 s default (raisable for Regional REST APIs)', ro: '29 s implicit (poate fi crescut la REST API Regional)' } },
      { label: { en: 'Default account throttle', ro: 'Throttle implicit pe cont' }, value: { en: '10,000 req/s, burst 5,000', ro: '10.000 req/s, burst 5.000' } },
      { label: { en: 'Max payload', ro: 'Payload maxim' }, value: { en: '10 MB', ro: '10 MB' } },
      { label: { en: 'Endpoint types', ro: 'Tipuri de endpoint' }, value: { en: '3 (Edge-Optimized, Regional, Private)', ro: '3 (Edge-Optimized, Regional, Private)' } },
    ],
    whenToUse: [
      { en: 'Serverless REST API: API Gateway + Lambda + DynamoDB — auth, throttling and versioning handled for you.', ro: 'REST API serverless: API Gateway + Lambda + DynamoDB — auth, throttling și versionare rezolvate din oficiu.' },
      { en: 'Put rate limiting, API keys, caching or auth in front of ANY HTTP backend, including on-premises.', ro: 'Pui rate limiting, API keys, caching sau auth în fața ORICĂRUI backend HTTP, inclusiv on-premises.' },
      { en: 'An API that must be reachable only inside VPCs → Private endpoint + interface VPC endpoint + resource policy.', ro: 'Un API care trebuie accesibil doar din VPC-uri → endpoint Private + interface VPC endpoint + resource policy.' },
    ],
    whenNotToUse: [
      { en: 'Long-running requests (> 29 s by default) → the answer is async: queue the work (SQS) or use Step Functions, return immediately.', ro: 'Cereri de lungă durată (> 29 s implicit) → răspunsul e async: pui treaba în coadă (SQS) sau folosești Step Functions și răspunzi imediat.' },
      { en: 'Simple HTTPS in front of containers/EC2 without API management features → an ALB is cheaper.', ro: 'HTTPS simplu în fața containerelor/EC2 fără funcții de API management → un ALB e mai ieftin.' },
    ],
    examTraps: [
      { en: 'Edge-Optimized custom domain → the ACM certificate MUST be in us-east-1 (CloudFront rule). Regional → same region as the API.', ro: 'Domeniu custom Edge-Optimized → certificatul ACM TREBUIE să fie în us-east-1 (regula CloudFront). Regional → aceeași regiune cu API-ul.' },
      { en: '"Expose the API only inside the VPC" → Private endpoint type + interface VPC endpoint + resource policy — not security groups alone.', ro: '„Expune API-ul doar în interiorul VPC-ului” → endpoint Private + interface VPC endpoint + resource policy — nu doar security groups.' },
      { en: 'The 29 s integration timeout is the trap behind "the API times out on long requests" — decouple with SQS/Step Functions instead of raising Lambda memory.', ro: 'Timeout-ul de integrare de 29 s e capcana din „API-ul dă timeout la cereri lungi” — decuplezi cu SQS/Step Functions, nu crești memoria Lambda.' },
      { en: 'External/mobile user sign-in for an API → Cognito User Pools; internal AWS-credentialed callers → IAM auth; custom token logic → Lambda authorizer.', ro: 'Login utilizatori externi/mobili pentru un API → Cognito User Pools; apelanți interni cu credențiale AWS → auth IAM; logică de token custom → Lambda authorizer.' },
    ],
    retrievalQuestions: [
      { q: { en: 'What are the three API Gateway endpoint types and when does each fit?', ro: 'Care sunt cele trei tipuri de endpoint API Gateway și când se potrivește fiecare?' }, a: { en: 'Edge-Optimized (global clients, routed via CloudFront edges), Regional (clients in the same region, combine with your own CloudFront if wanted), Private (only via interface VPC endpoint).', ro: 'Edge-Optimized (clienți globali, rutat prin edge-urile CloudFront), Regional (clienți din aceeași regiune, combini cu CloudFront propriu dacă vrei), Private (doar prin interface VPC endpoint).' } },
      { q: { en: 'Where must the ACM certificate live for an Edge-Optimized custom domain?', ro: 'Unde trebuie să stea certificatul ACM pentru un domeniu custom Edge-Optimized?' }, a: { en: 'In us-east-1, because Edge-Optimized APIs sit behind CloudFront. Regional APIs need the cert in their own region.', ro: 'În us-east-1, pentru că API-urile Edge-Optimized stau în spatele CloudFront. Cele Regional au nevoie de cert în propria regiune.' } },
      { q: { en: 'A request needs 2 minutes of processing — how do you design it behind API Gateway?', ro: 'O cerere are nevoie de 2 minute de procesare — cum o proiectezi în spatele API Gateway?' }, a: { en: 'Asynchronously: API Gateway sends the job to SQS or starts a Step Functions workflow and returns immediately; the default 29 s integration timeout rules out synchronous processing.', ro: 'Asincron: API Gateway trimite job-ul în SQS sau pornește un workflow Step Functions și răspunde imediat; timeout-ul de integrare implicit de 29 s exclude procesarea sincronă.' } },
      { q: { en: 'What are the three auth options for API Gateway?', ro: 'Care sunt cele trei opțiuni de auth pentru API Gateway?' }, a: { en: 'IAM (internal apps with AWS credentials), Cognito User Pools (external/mobile users), Lambda/custom authorizer (your own token logic).', ro: 'IAM (aplicații interne cu credențiale AWS), Cognito User Pools (utilizatori externi/mobili), Lambda/custom authorizer (logica ta de token).' } },
    ],
  },
  {
    id: 'elb',
